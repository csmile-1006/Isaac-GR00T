...
@dataclass
class IQLCriticConfig(PretrainedConfig):
...
class IQLCritic(nn.Module):
    ...
    def compute_value_loss(
        ...
        vs = self.value(vl_embed_features, action_input.state)
        ...
        value_loss = expectile_loss(q - vs, q - vs, self.rl_config.expectile).mean()
    ...
    def compute_critic_loss(
        ...
        with torch.no_grad():
            ...
            vs = self.value(next_vl_embed_features, action_input.next_state)
            target_v = (
                scaled_rewards
        ...
        critic_loss = ((q1 - target_v)**2 + (q2 - target_v)**2).mean()
        metrics = {
