...
@dataclass
class OurActionHeadAWRConfig(PretrainedConfig):
...
class OurActionHeadAWR(nn.Module):
    config_class = OurActionHeadAWRConfig
    supports_gradient_checkpointing = True

    def __init__(
        self,
        config: OurActionHeadAWRConfig,
    ):
        super().__init__()
    ...
    def flow_matching_loss(self, backbone_output: BatchFeature, action_input: BatchFeature) -> tuple[torch.Tensor, dict[str, torch.Tensor]]:
        # Set frozen modules to eval
        self.set_frozen_modules_to_eval_mode()

        flow_matching_metrics = {}
        ...
        # Compute advantage
        vl_embeds_mean = vl_embs.mean(dim=1)
        ...
        adv = q - vs
        exp_a = torch.exp(adv * self.rl_config.alpha)
        exp_a = torch.minimum(exp_a, torch.ones_like(exp_a) * 100)

        # Slice out only the action portion of pred and target.
        action_mask = action_input.action_mask
        loss = exp_a[..., None, None] * F.mse_loss(pred_actions, velocity, reduction="none") * action_mask
        loss = loss.sum() / action_mask.sum()
        return loss, flow_matching_metrics

    def compute_flow_matching_loss(self, backbone_output: BatchFeature, action_input: BatchFeature) -> tuple[torch.Tensor, dict[str, torch.Tensor]]:
        """Compute flow matching loss with proper gradient isolation."""
        # Set frozen modules to eval
        self.set_frozen_modules_to_eval_mode()

        backbone_output = self.process_backbone_output(backbone_output)
        loss, flow_matching_metrics = self.flow_matching_loss(backbone_output, action_input)
        return loss, flow_matching_metrics
    ...
    def forward(
        ...
        flow_matching_loss, flow_matching_metrics = self.compute_flow_matching_loss(backbone_output, action_input)
        ...
        output_dict = {
            ...
            **{f"flow_matching/{k}": v for k, v in flow_matching_metrics.items()},
