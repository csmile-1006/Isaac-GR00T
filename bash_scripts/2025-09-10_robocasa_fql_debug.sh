...
python scripts/gr00t_fql_finetune.py \
    ...
    --num-gpus 2 \
    --output-dir ~/debug/gr00t_n1_5_fql_ckpt/step${STEPS}_${TASK_NAME}_num${NUM_DEMOS}_num${NUM_ROLLOUTS} \
    --max-steps ${STEPS} \
    --data-config single_panda_gripper_rl_as1 \
    --batch-size 8 \
    --save-steps 5000 \
    --run-name debug_${TASK_NAME}_num${NUM_DEMOS}_num${NUM_ROLLOUTS} \
    # --lora_rank 64 \
    # --lora_alpha 128
