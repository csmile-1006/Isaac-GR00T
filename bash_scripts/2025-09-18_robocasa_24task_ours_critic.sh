#!/bin/bash                                                                                                                                                  
#SBATCH --job-name=critic_grn15_rc_24
#SBATCH --output=/home/changyeon/slurm-logs/critic_grn15_rc_24/%A_%a.out  # log                                                                                                   
#SBATCH --error=/home/changyeon/slurm-logs/critic_grn15_rc_24/%A_%a.err   # log                                                                                                   
...
RUN_NAME=OURS_Critic_as${CRITIC_ACTION_HORIZON}_e${EXPECTILE}_d1${DISCOUNT1}_d2${DISCOUNT2}_bs${TOTAL_BATCH_SIZE}_steps${RUN_NAME}_dm${NUM_DEMOS}_roll${NUM_ROLLOUTS}
python scripts/gr00t_ours_critic_finetune.py \
    --dataset-path ${dataset_path} \
    --num-gpus ${NUM_GPUS} \
    --output-dir ${BASE_PATH}/ckpts/24task/${RUN_NAME}/ \
    --max-steps ${STEPS} \
