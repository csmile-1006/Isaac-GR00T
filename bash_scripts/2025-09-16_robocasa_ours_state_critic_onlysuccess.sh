#!/bin/bash                                                                                                                                                  
#SBATCH --job-name=scritic_grn15_rc_succ
#SBATCH --output=/home/changyeon/slurm-logs/scritic_grn15_rc_succ/%j.out  # log                                                                                                   
#SBATCH --error=/home/changyeon/slurm-logs/scritic_grn15_rc_succ/%j.err   # log                                                                                                   
...
RUN_NAME=OURS_State_Critic_${CKPT_TYPE}_as${CRITIC_ACTION_HORIZON}_e${EXPECTILE}_d1${DISCOUNT1}_d2${DISCOUNT2}_bs${TOTAL_BATCH_SIZE}_steps${RUN_NAME}_dm${NUM_DEMOS}_roll${NUM_ROLLOUTS}_succ
python scripts/gr00t_ours_state_critic_finetune.py \
    --dataset-path ${BASE_PATH}/data/robocasa_dataset/${TASK_NAME}_num${NUM_DEMOS}/ ${BASE_PATH}/gr00tn15_robocasa/rollout_demos/${TASK_NAME}_num${NUM_ROLLOUTS}/lerobot \
